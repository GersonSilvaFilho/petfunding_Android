...
class ChatModule(val chatActivity: ChatActivity) {
    @Provides
    fun provideMainChatPresenter(chatRepository: ChatRepository, userRepository: UserRepository, matchReposity: MatchReposity): ChatContract.Presenter{
        return ChatPresenter(chatActivity, chatRepository, userRepository, matchReposity)
    }
}